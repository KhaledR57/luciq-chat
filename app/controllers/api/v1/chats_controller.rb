class Api::V1::ChatsController < ApplicationController
  ...
  def index
    response = ""
    page = params[:page] || 0   
    rkey = "#{@my_application.id}_#{page}_chats"
    
    if $redis.exists(rkey) != 0 and Rails.env.production?
      response = $redis.get(rkey)
    else
      @chats = page == 0 ? @my_application.chats.all : @my_application.chats.page(page)
      response = (@chats.as_json())
  ...
  def show
    if @chat
      @chat.messages_count = $redis.get("messages_count&#{@chat.id}").to_i
      render json: @chat
    else
